...
function clean_queue_part2(){
    write_queue();
    currently_playing=0;
    document.getElementById("music-element").setAttribute('src', songs[currently_playing].song);
    music.pause();
    handlePlay();
}
...
function next_song() {
if(currently_playing+1 == songs.length)
currently_playing=0;
else  
currently_playing++;    document.getElementById("curr_name").innerHTML=songs[currently_playing].songName;
document.getElementById("artist_name").innerHTML=songs[currently_playing].artistName;
document.getElementById("current_img").setAttribute('src', songs[currently_playing].img);  
document.getElementById("music-element").setAttribute('src', songs[currently_playing].song);
handlePlay();  
}  
...
currently_playing=songs.length-1;
else  
currently_playing--;    document.getElementById("curr_name").innerHTML=songs[currently_playing].songName;
document.getElementById("artist_name").innerHTML=songs[currently_playing].artistName;
document.getElementById("current_img").setAttribute('src', songs[currently_playing].img);
...
function handlePlay() {
  document.getElementById("curr_name").innerHTML=songs[currently_playing].songName;
  document.getElementById("artist_name").innerHTML=songs[currently_playing].artistName;
  document.getElementById("current_img").setAttribute('src', songs[currently_playing].img); 
    if (music.paused) {
        music.play();
